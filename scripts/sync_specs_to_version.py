#!/usr/bin/env python3
"""Merge OpenSpec change deltas into a versioned baseline (e.g. v2.0.0.md).
...
def sync_to_version(
    ...
    version_label = target_version.removeprefix("v")
    header = [
        f"# 胖宝 OpenSpec 基线 {target_version}",
        "",
    ...
    body_parts = [_render_capability(name, _capability_requirements(capabilities[name])) for name in sorted(capabilities)]
    out_path = specs_dir / f"{target_version}.md"
    out_text = "\n".join(header) + "\n" + "\n---\n\n".join(body_parts) + "\n\n---\n"
    out_path.write_text(out_text, encoding="utf-8")
    print(f"Wrote {out_path} ({out_path.stat().st_size:,} bytes, {len(capabilities)} capabilities, {len(applied)} deltas)")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Sync OpenSpec change deltas into a version baseline.")
    parser.add_argument("version", help="Target version label, e.g. v2.0.0")
    parser.add_argument(
        "--base",
        help="Base version file name without path, e.g. v1.0.1 (default: latest v*.md)",
    )
    parser.add_argument(
        "--remove-changes",
        action="store_true",
