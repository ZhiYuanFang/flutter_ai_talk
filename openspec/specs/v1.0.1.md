...
> 由 `openspec/specs/**/spec.md` 共 **111** 个 capability 合并生成。
> 生成日期：2026-06-06
...
## Purpose
Sign in with Apple on iOS alongside existing login methods via the apple_login gateway.
...
# apple-wechat-account-bind Specification

## Purpose
Optional binding of a second auth provider (Apple or WeChat) to the current account.
...
# device-identity-sync Specification

## Purpose
Ensure baby ID (`deviceNo`) persistence, broadcast, and service consumption stay consistent, including JWT alignment after bind.
...
# history-ws-token-sync-after-bind Specification

## Purpose
Force token refresh after baby bind so history WebSocket auth uses a JWT with the latest `device_no`.
...
# home-history-ws-status-banner Specification

## Purpose
Inline history WebSocket status banner on the home screen with reconnect affordance; hide during auto-reconnect.
...
# ios-adhoc-test-api-base Specification

## Purpose
TBD - created by archiving change adhoc-test-api-base-url. Update Purpose after archive.
## Requirements
...
# ios-microphone-usage-string Specification

## Purpose
App Store compliant NSMicrophoneUsageDescription with concrete parenting voice example.
...
# ios-wechat-login-visibility Specification

## Purpose
Conditionally show WeChat login based on installation; no web OAuth fallback.
...
# user-facing-baby-id-terminology Specification

## Purpose
Use baby-ID user-facing copy instead of device terminology, including mapped API and WebSocket errors.
...
# username-password-auth Specification

## Purpose
Username-password sign-in alongside conditional WeChat and Sign in with Apple on iOS.
