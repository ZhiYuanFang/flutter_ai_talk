...
> 由 `v2.0.2.md` 合并 **78** 条 change delta 生成，共 **179** 个 capability。
> 生成日期：2026-06-29
...
While the square recommended or following feed is loading and waiting for location consent or GPS, the client MUST display a user-visible status message indicating the current step (e.g. obtaining location vs loading feed).
...
- **WHEN** the feed has loaded but location coordinates are not yet cached
- **AND** the user pull-to-refreshes
- **THEN** the client MUST still show phase status messages while resolving location and loading feed
...
On iOS and Android, `ucgUploadLocalVideo` MUST transcode every local video before upload (no size-based skip). Output MUST be MP4 with H.264 **Main** profile at **level 3.1** or lower compatible level, **yuv420p** with **BT.709** color metadata (`colorspace`/`color_primaries`/`color_trc` MUST NOT remain source-exotic values such as `gbr`), max duration 15 seconds, max dimension 1280px width (preserve aspect, even dimensions), 30 fps, `-movflags +faststart`. Video filter chain MUST include `format=yuv420p` after scale. If source has no audio track, output MUST include a silent AAC stereo track. Output size MUST target `UcgMediaLimits.videoMaxBytes` (20MB); if first pass exceeds limit, client MAY retry with lower CRF or smaller scale before failing with user-visible error. **`transform_version` MUST remain `"v2"`** for native normalized uploads (algorithm fix without version bump).

iOS/Android 上 MUST 对每条本地视频 ffmpeg 转码。输出 MUST：H.264 Main level≤3.1、yuv420p、**BT.709 色域**（不得保留 gbr 等异常标记）、≤15s、宽≤1280、30fps、faststart；scale 后 MUST `format=yuv420p`。无音轨 MUST 补静音 AAC。超出 20MB MUST 降参重试。**native 仍使用 `transform_version` `v2`**。

#### Scenario: Normalized output has BT.709 not gbr

- **WHEN** native normalize 成功完成
- **THEN** 输出 MP4 经 ffprobe 或等价校验 MUST NOT 报告 `color_space=gbr`
- **AND** SHOULD 报告 bt709 相关色域标记
...
The go_ai_talk service (out of Flutter repo scope) MUST eventually transcode Web-originated `transform_version` `v1` video blobs to the same canonical format as native v2 (BT.709, H.264 Main, AAC, faststart, ≤15s, ≤20MB target). Until server transcode completes, Flutter MAY show list thumbnail; inline Web playback MAY fail without external-player fallback (see removed `ucg-video-playback-fallback`).

go_ai_talk 服务端 MUST（后续）将 Web v1 转码为与 native v2 相同 canonical 格式；完成前 Web 内联 MAY 失败，**不再**要求外链兜底。
...
### Requirement: Native normalize SHALL pass ExoPlayer probe before upload

On iOS and Android, after successful ffmpeg normalize and before OSS upload, the client MUST verify the temporary output MP4 is playable by the same in-app stack used for CDN inline video (`VideoPlayerController` file initialize, with bounded timeout). If probe fails, the client MUST NOT upload; MUST surface user-visible upload failure and emit `[UcgVideo]` probe failure log.

原生 normalize 成功后、上传前 MUST 用 App 内 ExoPlayer 对临时 MP4 试 init；失败 MUST 阻止上传并记录日志。

#### Scenario: Probe fail blocks upload

- **WHEN** normalize 输出文件 ExoPlayer initialize 失败
- **THEN** `ucgUploadLocalVideo` MUST throw 或返回失败
- **AND** compose/chat 槽位 MUST 标记 failed
- **AND** MUST NOT 调用 resolve/register 上传

#### Scenario: Probe ok allows upload

- **WHEN** normalize 输出 ExoPlayer initialize 成功
- **THEN** Client SHALL 继续 hash 计算与 v2 上传
---
