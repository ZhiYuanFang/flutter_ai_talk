# 胖宝 OpenSpec 基线 v2.0.3

> 由 `v2.0.3.md` 合并 **5** 条 change delta 生成，共 **181** 个 capability。
> 生成日期：2026-07-01
...
The home screen SHALL display a full-width inline banner between the history list region and the bottom input panel when the history WebSocket is not ready and phase-specific rules apply, with **graded copy and styling** by session refresh state and WS phase. 当历史 WebSocket 未就绪时，主页**必须**在历史列表与底部输入区之间按 **refresh 状态与 WS phase 分级**展示全宽内联横幅（文案与样式分级如下）：

| 条件 | 文案 | 样式 / 交互 |
|------|------|-------------|
| `isRefreshInFlight == true` | **「正在恢复连接…」** | 信息态（非 errorContainer）；**不可点击**重连 |
| `autoReconnecting` | （隐藏） | **不得**展示任何历史 WS 连接状态横幅 |
| `gaveUp` 且已登录 | **「连接失败，请检查网络后点击重连」** | 错误/警示态；可点击重连 |
| `disconnected` 且已登录、非 refresh | **「连接中断，请点击重连」** | 错误/警示态；可点击重连 |
| 未登录或未绑定宝宝 | （隐藏） | — |

**`autoReconnecting` 期间 MUST NOT 展示任何历史 WS 连接状态横幅**（含「正在重连…」）。refresh 明确失败导致未登录时**不得**展示 WS 横幅（由登录空态与「登录已过期，请重新登录」Toast 承担）。
...
#### Scenario: Informational banner during token refresh in flight

- **WHEN** 用户已登录且已绑定宝宝，且 `SessionController.isRefreshInFlight == true`，且历史 WS 尚未就绪
- **THEN** **必须**展示文案 **「正在恢复连接…」** 的信息态横幅
- **AND** **不得**展示「连接中断，请点击重连」或「连接失败，请检查网络后点击重连」
- **AND** 该横幅**不得**响应点击重连（`tapEnabled == false`）
...
- **THEN** 历史列表与输入区之间**必须**显示 **「连接中断，请点击重连」** 横幅（可点击重连）
...
The home screen SHALL display banner **「连接失败，请检查网络后点击重连」** and a one-time snackbar when history WebSocket phase is `gaveUp` **and** `isRefreshInFlight == false` (and not superseded by autoReconnecting hide rule). 当历史 WebSocket phase 为 **`gaveUp`** 且 **`isRefreshInFlight == false`** 时，主页**必须**展示 **「连接失败，请检查网络后点击重连」** 横幅，并**必须**弹出**一次性** Snackbar（同主题文案）。`isRefreshInFlight == true` 期间**不得**触发 gaveUp Snackbar。进入 `autoReconnecting` 后**必须**按「自动重连隐藏横幅」隐藏，直至再次进入 gaveUp 或 disconnected。
...
- **AND** **必须**改为展示 refresh 信息态横幅（「正在恢复连接…」）
...
The refresh in-flight banner SHALL use informational styling distinct from error/disconnect banners so users understand session recovery is in progress rather than a hard failure. refresh 进行中的「正在恢复连接…」横幅**必须**采用与错误/断开横幅可区分的**信息态**视觉（如非 `errorContainer` 底色、无 cloud_off 错误图标），使用户识别为会话恢复中而非硬失败。

#### Scenario: Readable informational banner on shell background

- **WHEN** `isRefreshInFlight == true` 且 refresh 信息态横幅可见
- **THEN** 文案与图标**必须**满足可读对比度（浅色/深色主题下均可见）
- **AND** **不得**展示可点击的 refresh 重连图标（与 disconnected/gaveUp 错误横幅区分）
---
---
---
...
When `session_sync` delivers non-empty `turns`, the client MUST rebuild Q&A from server data as authoritative for `question` and `answer`, and MUST preserve locally known `thinking` for matching turns when server data omits thinking. 当 `session_sync` 含非空 `turns` 时，客户端**必须**以服务端数据为 `question`/`answer` 权威重建列表，并对匹配轮次**必须**保留本地已知 `thinking`（服务端不含 thinking 时）。
...
When the clinic chat has no messages, the client MUST show an empty-state gallery (animation or icon, title, subtitle). If the user is not logged in, the gallery MUST include a login action. If logged in without device binding, MUST include bind-baby action.

#### Scenario: Guest opens clinic

- **WHEN** the user opens 胖宝诊疗 without a session
...
When a clinic question turn fails with a WebSocket error during streaming, the Pangbao clinic screen MUST retain the assistant message row and display an inline error state with the parsed user message instead of removing the assistant bubble. 当进行中的诊疗轮次收到 WS error 时，页面**必须**保留助手消息行并以 **inline error** 展示解析后的文案；**不得**调用 `removeAssistant` 静默删除助手气泡。
...
For business codes 40301 and 40302, the client MUST continue to show the existing glass confirm dialog via `handleAiQuotaBusinessCode`, and MUST also set the assistant inline error summary consistent with the dialog message. 对 40301/40302，客户端**必须**继续展示现有 Glass 弹框，且**必须**在助手行 inline 展示与弹框一致的摘要文案。

#### Scenario: Quota exhausted 40302

- **WHEN** WS error 帧 `code == 40302`
- **THEN** **必须**弹出「本月额度已用完」类弹框
- **AND** assistant inline **必须**展示等价摘要（如「本月额度已用完」）

#### Scenario: Not logged in 40301

- **WHEN** WS error 帧 `code == 40301`
- **THEN** **必须**弹出登录引导弹框
...
The Pangbao clinic screen SHALL display a full-width inline banner below the app bar region and above the conversation list when the clinic WebSocket is not ready and phase-specific rules apply, with **graded copy and styling** aligned with the home history WebSocket status banner. 当胖宝诊疗 WebSocket 未就绪时，诊疗页**必须**在 AppBar 与对话列表之间按 **session refresh 状态与 WS phase 分级**展示全宽内联横幅（规则与主页历史 WS 横幅对齐）：

| 条件 | 文案 | 样式 / 交互 |
|------|------|-------------|
| 未同意告知 / 未登录 / 未绑定宝宝 | （隐藏） | — |
| `isRefreshInFlight == true` 且 WS 未就绪 | **「正在恢复连接…」** | 信息态；**不可点击**重连 |
| `autoReconnecting` | （隐藏） | **不得**展示诊疗 WS 连接状态横幅 |
| `gaveUp` 且已登录、已绑定 | **「连接失败，请检查网络后点击重连」** | 错误/警示态；可点击重连（reset strike） |
| `disconnected` 且已登录、已绑定、非 refresh | **「连接中断，请点击重连」** | 错误/警示态；可点击重连 |
| WS 已就绪 | （隐藏） | — |

`autoReconnecting` 期间 **MUST NOT** 展示任何诊疗 WS 连接状态横幅（含「正在重连…」）。refresh 明确失败导致未登录时**不得**展示 WS 横幅（由空态与登录 Toast 承担）。
...
- **THEN** AppBar 与对话列表之间**必须**显示 **「连接中断，请点击重连」** 横幅（可点击重连）
...
`ClinicWsClient` MUST expose synchronous and streaming clinic WebSocket readiness and phase state derived from the internal `ResilientWebSocketClient`, mapped to `HistoryWsPhase` for UI consumption. `ClinicWsClient` **必须**向 UI 暴露基于内部 `ResilientWebSocketClient` 的同步/流式就绪态与 phase（映射为 `HistoryWsPhase`），供 `PangbaoAiScreen` 驱动横幅。
...
- **AND** `clinicWsPhase` getter **必须**反映当前 phase

#### Scenario: Ready stream reflects auth_ok readiness

- **WHEN** clinic WS 完成 auth 且 transport 标记 ready（诊疗通道不要求 pong）
- **THEN** `clinicWsReadyStream` **必须**推送 `true`
- **AND** `isClinicWebSocketReady` **必须**为 `true`

#### Scenario: Manual reconnect API unchanged

- **WHEN** UI 调用 `ClinicWsClient.reconnect(resetStrike: true)`
- **THEN** **必须**委托内部 `ResilientWebSocketClient.reconnect`
- **AND** 既有 question/cancel 行为**不得**回归破坏
...
The compose screen SHALL allow: (a) text + up to 9 images, OR (b) text + 1 video with max duration 15 seconds and max size 20MB. User MUST NOT submit both multi-image set and video in one post. Media MUST be displayed in a 3×3 grid on the compose page using local preview for pending slots and network preview for remote-only slots. **Video** MUST be displayed as an **orientation-aware preview card** when a video is selected; when **no** video is selected, the compose screen MUST NOT show a separate empty video add tile alongside the image grid add tile. Images MUST support drag reorder; dragging to the bottom delete zone MUST remove the image and trigger orphan OSS delete when upload completed. User MAY add more images from the compose page when in image mode. When **editing an existing post** with **no images and no video**, the compose screen MUST show **exactly one** media add control that opens the same entry bottom sheet as new-post flow (`showUcgComposeEntrySheet`); the user MUST NOT see two concurrent add controls (image grid empty add + video empty add). After the user removes the only video while editing, with no images remaining, the same **single** entry-sheet add control MUST appear for re-selection. New-post compose (not editing) MUST NOT use the dual empty add pattern; first-time video selection for new posts remains via tab entry sheet, not a second compose video add tile.

发布页须支持正文+最多9图或正文+单视频；互斥；有视频时展示方向感知预览卡。**无视频时不得**与图片九宫格空态「+」并列展示第二个视频空态「+」。**编辑帖且无图无视频时**必须**仅展示一个**添加入口，点击打开与新发相同的 entry sheet；**不得**同时出现两个「+」。编辑删视频且无图后须同样仅一个入口重选。新发 compose 仍经 Tab 入口选视频，compose 内不得出现双空态添加。
...
#### Scenario: 编辑纯文字帖单一添加入口

- **WHEN** 用户从详情进入编辑 compose，且帖子当前无图片、无视频
- **THEN** compose **必须**仅展示 **一个** 媒体「+」控件
- **AND** 点击该控件 **必须**打开 entry bottom sheet（拍摄/相册）
- **AND** **不得**同时展示九宫格空态「+」与视频区空态「+」

#### Scenario: 编辑删视频后单一入口重选

- **WHEN** 用户编辑带视频帖子并删除唯一视频，且当前无图片
- **THEN** compose **必须**仅展示一个 entry sheet 添加入口
- **AND** 用户 ** MAY **通过 sheet 选择替换视频或改为图片（仍须满足图/视频互斥）

#### Scenario: 编辑已有图片仅九宫格追加

- **WHEN** 用户编辑已有图片的帖子
- **THEN** compose **必须**展示图片九宫格及追加「+」（在未满 9 张时）
- **AND** **不得**展示空态视频添加 tile

#### Scenario: 新发无媒体 compose

- **WHEN** 用户新发进入 compose 且尚未选择媒体
- **THEN** compose ** MAY** 展示九宫格选图「+」
- **AND** **不得**展示独立视频空态「+」（视频仍由 Tab 入口 sheet 首次带入）

#### Scenario: 编辑帖追加媒体

- **WHEN** 用户从详情进入编辑 compose 并通过 entry sheet 或九宫格追加本地图片或视频
- **THEN** 新媒体 SHALL 立即本地预览且后台上传
- **AND** 视频预览 SHALL 遵循方向感知预览卡条款
...
- **WHEN** 已登录 wx 已绑定用户停留喂养 page 0 且 WS 收到他人 `message_delivered`（`senderWxId` 非当前用户）
- **THEN** 客户端 SHALL 立即将 `ucgUnreadCountProvider` 加 1（或等价递增）
- **AND** 「进入广场」拉条 SHALL 在不发起同步 HTTP 的情况下显示红点

#### Scenario: WS 互动通知即时亮红点

- **WHEN** 用户停留喂养 page 0 且 WS 收到 `comment_notification`
...
- **WHEN** 冷启动进入喂养 page 0 且服务端存在历史未读（非本次 WS 新消息）
