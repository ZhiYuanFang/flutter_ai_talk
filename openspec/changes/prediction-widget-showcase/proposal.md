...
桌面小组件已落地，但预测主路径缺少可发现入口，用户不易理解「可在桌面看预测」。需要在预测页竖屏用悬浮入口秀出能力，并按是否已钉小组件分流引导。
...
- `prediction-widget-showcase`：预测竖屏入口、安装态分流、large Flutter 预览、刷新闸门与文案。
...
- UI：`smart_prediction_screen.dart` 竖屏 Stack FAB；新 screen + go_router 路由。
